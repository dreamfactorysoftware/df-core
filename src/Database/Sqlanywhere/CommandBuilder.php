...
/**
 * CommandBuilder provides basic methods to create query commands for tables for Sap Servers.
 */
class CommandBuilder extends \DreamFactory\Core\Database\CommandBuilder
{
     ...
     * Override parent implementation because Sap don't want to update an identity column
     *
     * @param TableSchema $table    the table metadata
    ...
    public function applyLimit($sql, $limit, $offset)
    {
        ...
        if ($limit > 0 && $offset <= 0) //just limit
        {
            ...
            $offset++;
            $sql =
                preg_replace('/^([\s(])*SELECT( DISTINCT)?(?!\s*TOP\s*\()/i',
                    "\\1SELECT\\2 TOP $limit START AT $offset", $sql);
//            $sql = $this->rewriteLimitOffsetSql($sql, $limit, $offset);
    ...
    /**
     * Rewrite sql to apply $limit > and $offset > 0 for Sap database.
